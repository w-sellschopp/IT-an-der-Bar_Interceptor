    ...
    if [ -f /etc/apt/apt.conf.d/20auto-upgrades ]; then
        if grep -q 'APT::Periodic::Unattended-Upgrade "1"' /etc/apt/apt.conf.d/20auto-upgrades; then
            if [ -f /etc/apt/apt.conf.d/50unattended-upgrades ]; then
                ...
                # vorhanden sind, die nicht auskommentiert sind, dann "aktiv", ansonsten "Sicherheitspatches"
...
EOF
  ...
  *)
    echo "Ungültige Auswahl. Bitte führen Sie das Skript erneut aus und wählen Sie eine gültige Option (1-3)."
    exit 1
    ;;
esac
